 ...
 */
function startBackendCore(userDataPath, port) {
    ...
    let logLevel = process.env.CANOPY_LOG_LEVEL || 'INFO';
    ...
    let docsPath = path.join(__dirname, '..', 'public', 'docs', 'manual');
    let changelogPath = path.join(__dirname, '..', 'public', 'docs', 'changelog.md');
    if (isPackaged) {
        docsPath = path.join(process.resourcesPath, 'app.asar.unpacked', 'dist', 'docs', 'manual');
        changelogPath = path.join(process.resourcesPath, 'app.asar.unpacked', 'dist', 'docs', 'changelog.md');
    }
    ...
    if (!isPackaged) {
        ...
        // For development, use a persistent data directory isolated from production
        dataPath = `${userDataPath}-DEV`;
    } else {
    ...
    coreProcess = spawn(binaryPath, [], {
        ...
        env: {
            ...process.env,
            "CANOPY_HOST": "127.0.0.1",   // Ensure strict loopback binding
            "CANOPY_TOKEN": sessionToken, // Inject the token strictly via memory isolation
            "CANOPY_PORT": port.toString(),
            "CANOPY_DATA_PATH": dataPath,
            "CANOPY_PORTABLE_MODE": isPortableMode,
            "CANOPY_LOG_LEVEL": logLevel,
            "CANOPY_DOCS_PATH": docsPath,
            "CANOPY_CHANGELOG_PATH": changelogPath
        },
        windowsHide: true // Prevents flashing a terminal window on desktop environments
    });

    // Catch binary execution failures (e.g. missing file, permission denied)
    coreProcess.on('error', (err) => {
        sysLog('Lifecycle Error', `Failed to start Go daemon: ${err.message}`, true);
    });
