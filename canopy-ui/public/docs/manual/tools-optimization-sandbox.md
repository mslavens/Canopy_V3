...
- **Select Object Dropdown**: Click inside the input box to open the dropdown. Here you can search through existing objects and use the tab bar to quickly filter between **Objects** and **Groups**. You can also click the **Selected** tab to view only the items you've checked, and easily reset your view by clicking the **All** tab. Finally, use the **Select All Filtered** checkbox for rapid bulk addition of whatever items are currently visible in your active tab. The dropdown automatically honors your selected Administrative Scope hierarchy to ensure you only see valid objects.
