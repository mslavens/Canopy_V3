...
- **Rollbacks & Migrations:** In production environments, database schema mutations must be strictly additive (Forward-Only). Dropping columns or renaming tables is prohibited, ensuring that applying an older `.cpatch` file (a downgrade) will never crash against a newer database schema. During active local development, deleting the local database entirely to rebuild the schema from scratch is permitted for rapid iteration, but production migrations must remain strictly additive.
...
- **Layout & Spacing Standardization:**
  ...
  - **Search Bars:** All global and page-level search bars must use a consistent default width of `250px`. They must also explicitly declare a `historyKey` (e.g. `historyKey="table-search-history"` or an isolated key like `historyKey="resolver-sandbox-history"`) to enable persistent search history.
